 ...
 *
 * <p>
 * No conoce lógica de negocio — solo sabe dibujar lo que le pasan.
 * </p>
 *
...
public class MapaView {
    ...
    /**
     * Callback que se dispara cuando el usuario presiona y arrastra (para
     * desactivar cámara)
     */
    ...
    private void configurarPaneo() {
        ...
        contenedorMapa.setOnMouseReleased(event -> contenedorMapa.setCursor(javafx.scene.Cursor.DEFAULT));
    }
    ...
    private void ajustarLimites() {
        ...
        if (canvasMap.getTranslateX() > limX)
            canvasMap.setTranslateX(limX);
        if (canvasMap.getTranslateX() < -limX)
            canvasMap.setTranslateX(-limX);
        if (canvasMap.getTranslateY() > limY)
            canvasMap.setTranslateY(limY);
        if (canvasMap.getTranslateY() < -limY)
            canvasMap.setTranslateY(-limY);
    }
    ...
    /**
     * Canvas expuesto para que VentanaControl registre el click de selección
     * manual.
     */
    ...
    public void setOnMapaClicked(javafx.event.EventHandler<javafx.scene.input.MouseEvent> handler) {
        ...
        // Ahora es imposible que JavaFX ignore el evento, incluso si arrastrás el
        // mouse.
     ...
     *
     * <p>
     * Capas de dibujo (orden importa para que nada tape lo anterior):
     * </p>
     * <ol>
    ...
    public void dibujarVehiculosYRutas(ArrayList<Vehiculo> flota,
        ...
        for (Vehiculo v : flota) {
            ...
            if (dest != null && estaDentro(dest)) {
                graphCx.setFill(Color.web("#FFD700", 0.7));
        ...
        // --- CAPA 4: vehículos ---
        for (Vehiculo v : flota) {
            // ¡ARREGLO DE TELETRANSPORTACIÓN!
            // Ahora TODOS los estados usan la posición interpolada decimal.
            double x = lngAPixel(v.getLngDecimal());
            double y = latAPixel(v.getLatDecimal());
